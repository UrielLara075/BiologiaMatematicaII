globals [
  ...
  p               ;; probabilidad de propagar el fuego a vecinos
]
...
to go
  set p (exp (- Distancia)) * (Sequedad / 100)
  if not any? patches with [estado = 1]  ;; either fires or embers
    [ stop ]
  ask patches with [estado = 1]
    [ ask neighbors with [estado = 0] [
          if random-float 1 < p [ignite]]]
  fade-embers
  tick
end
...
to fade-embers
  ask patches with [estado = 1]
    [ set pcolor pcolor - 0.3  ;; make red darker
      if pcolor < red - 3.5     ;; are we almost at black?
        [ set pcolor pcolor
          set estado 2 ] ]
end
...
@#$#@#$#@
<experiments>
  <experiment name="UmbralDensidad" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(burned-trees / initial-trees)</metric>
    <enumeratedValueSet variable="Sequedad">
      <value value="100"/>
    </enumeratedValueSet>
    <steppedValueSet variable="Densidad" first="5" step="0.1" last="8"/>
  </experiment>
  <experiment name="UmbralSeparacion" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(burned-trees / initial-trees)</metric>
    <enumeratedValueSet variable="Sequedad">
    ...
    <steppedValueSet variable="Distancia" first="3.5" step="0.01" last="3.9"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
